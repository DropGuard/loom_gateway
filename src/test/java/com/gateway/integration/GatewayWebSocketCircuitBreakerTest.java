...
class GatewayWebSocketCircuitBreakerTest {
    ...
    /**
     * The core regression: a WebSocket upgrade completes asynchronously, so under
     * the old model every successful upgrade was counted as a failure and the
     * breaker opened after threshold successes. Here, repeated successes must keep
     * the breaker CLOSED (ws-chat has failureThreshold=3).
     */
    @Test
    void successfulUpgrades_doNotTripBreaker() throws Exception {
        for (int i = 0; i < 5; i++) {
            ...
            assertEquals("echo:hi", result.get(5, TimeUnit.SECONDS),
                    "WS upgrade #" + (i + 1) + " should succeed (breaker must stay closed)");
        }
    }

    /**
     * Repeated WS upgrade failures must still open the breaker. ws-fail points at
     * a port with no listener, so every upgrade fails.
     */
    @Test
