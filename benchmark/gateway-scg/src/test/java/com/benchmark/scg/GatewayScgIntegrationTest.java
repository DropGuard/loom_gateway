...
/**
 * End-to-end integration test for the SCG benchmark filter chain. Boots the full gateway (no
 * Docker) against an in-process WireMock stub upstream, and asserts every feature the symmetric
 * comparison advertises. These assertions exist precisely because each one would have caught a trap
 * we hit while building SCG:
 *
 * <p>- ResponseHeaders: a {@code getHeaders()}-decorator approach is bypassed by
 * NettyWriteResponseFilter, so the header would silently never appear. This test fails if that
 * regression returns. - Timeout: SCG 4.x removed the built-in RequestTimeout filter, so a
 * misconfigured name makes the route fail to start — also caught (context won't come up). -
 * ClaimsForward: depends on JwtAuth publishing the claims attribute; a broken wiring means
 * X-User-Id is absent downstream.
 */
...
class GatewayScgIntegrationTest {
    ...
    @Autowired CacheGatewayFilterFactory cacheFactory;
    ...
    void resetStub() {
        ...
        stub.resetAll();
        stub.stubFor(
                get(urlPathEqualTo("/bench/open/data"))
                        .willReturn(
                                aResponse()
                                        .withStatus(200)
                                        ...
                                        .withBody("{\"ok\":true}")));
        stub.stubFor(
                get(urlPathEqualTo("/bench/auth/data"))
                        .willReturn(
                                aResponse()
                                        .withStatus(200)
                                        .withHeader("Content-Type", "application/json")
        ...
        stub.stubFor(
                get(urlPathEqualTo("/bench/rl/data"))
                        .willReturn(
                                aResponse()
                                        .withStatus(200)
                                        .withHeader("Content-Type", "application/json")
                                        .withBody("{\"ok\":true}")));
        stub.stubFor(
                get(urlPathEqualTo("/bench/cache/data"))
                        .willReturn(
                                aResponse()
                                        .withStatus(200)
                                        .withHeader("Content-Type", "application/json")
        ...
        stub.stubFor(
                get(urlPathEqualTo("/echo-headers"))
                        .willReturn(aResponse().withStatus(200).withBody("{\"captured\":true}")));
    }
    ...
    void authRouteRejectsMissingToken() {
        webClient.get().uri("/bench/auth/data").exchange().expectStatus().isUnauthorized();
    }
    ...
    void authRouteForwardsSubAsXUserIdToUpstream() throws Exception {
        ...
        stub.verify(
                1,
                getRequestedFor(urlPathEqualTo("/bench/auth/data"))
    ...
    void openRouteForwardsNoIdentityWhenAnonymous() {
        webClient.get().uri("/bench/open/data").exchange().expectStatus().isOk();

        stub.verify(
                0,
                getRequestedFor(urlPathEqualTo("/bench/open/data"))
                        .withHeader("X-User-Id", equalTo("bench-user")));
    }
    ...
    void openRouteAddsBenchmarkResponseHeaders() {
        webClient
                ...
                .isOk()
                .expectHeader()
                .valueEquals("X-Gateway", "scg")
                .expectHeader()
                .exists("X-Request-Id")
                .expectHeader()
                .exists("X-Process-Time");
    }
                ...
                get(urlPathEqualTo("/bench/open/data"))
                        .willReturn(
                                aResponse()
                                        ...
                                        .withFixedDelay(1500)
                                        .withBody("{\"ok\":true}")));
        webClient.get().uri("/bench/open/data").exchange().expectStatus().isEqualTo(504);
    }
