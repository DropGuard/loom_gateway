...
/**
 * Per-route upstream timeout, mirroring the Loom Gateway benchmark's per-route
 * timeout filter. Wraps the filter chain in a {@link Mono#timeout(Duration)}
 * so an upstream that does not respond within {@code timeout} ms fails the
 * exchange with 504 — symmetric with Loom's {@code HttpProxyHandler} timeout.
 *
 * (Spring Cloud Gateway 4.x removed the built-in per-request timeout filter,
 * so this is a small custom factory rather than the old {@code RequestTimeout}
 * short name.)
 */
@Component
